...
import { UpdateActivitys } from "../api/activities";
...
export default function UpdateActivity({ route, navigation }) {
  ...
  const AddActivity = async () => {
    Keyboard.dismiss();
    if (validate()) {
      const res = await UpdateActivitys({
        ...ActivityInfos,
  ...
  const [date, setdate] = useState(
    new Date(route.params.infos.date).getFullYear() +
      "-" +
      (new Date(route.params.infos.date).getMonth() + 1) +
      "-" +
      new Date(route.params.infos.date).getDate()
  );
